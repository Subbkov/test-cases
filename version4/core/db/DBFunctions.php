 ...
 * @throws Exception
 */
function getQueueSubscriptionExpiringData(mysqli $mysqli, int $limit = 100): mysqli_result
{
    $sql = 'SELECT 
                u.username,
                u.email,
    ...
    $result = selectDb($mysqli, $sql, [$limit]);

    if (!$result instanceof mysqli_result) {
        throw new Exception('[mysqli] Ошибка получения данных из очереди');
    }
 ...
 * @throws Exception
 */
function addToQueueUserSubscriptionExpiration(mysqli $mysqli, array $userIds): void
{
    if (count($userIds) === 0) {
        return;
    }

    $sql = 'INSERT IGNORE INTO queue_user_subscription_expiration (user_id) VALUES ';
